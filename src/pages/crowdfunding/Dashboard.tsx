...
export default function Dashboard() {
  ...
  const [selectedNft, setSelectedNft] = useState<ProjectNftCatalogItemDTO | null>(null);
  ...
  const nftTokenByProjectId = useMemo(() => {
    ...
    for (const donation of successfulDonations) {
      ...
      if (!Number.isFinite(projectId) || !Number.isFinite(nftId) || nftId <= 0) continue;
  ...
  const ownedNftCount = useMemo(
    () =>
      catalog.filter((item) => ownedProjectIds.has(Number(item.projectId))).length,
    [catalog, ownedProjectIds],
  );
  const selectedNftExplorerUrl = selectedNft
    ? buildNftTokenExplorerUrl(nftTokenByProjectId.get(Number(selectedNft.projectId)))
    : null;
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {successfulDonations.length === 0 ? (
              <div className="bg-white p-12 rounded-[2.5rem] border border-slate-100 text-center space-y-4">
                <span className="material-icons text-6xl text-slate-300">favorite_border</span>
                <h3 className="text-2xl font-black uppercase tracking-tight text-slate-400">Nenhuma doacao confirmada</h3>
                <Link
                  to="/contribuir"
              ...
              <div className="grid grid-cols-1 gap-6">
                {successfulDonations.map((donation) => {
                  ...
                  const txExplorerUrl = buildTransactionExplorerUrl(donation.txHash);
                  const nftExplorerUrl = buildNftTokenExplorerUrl(Number(donation.nftId));
                  return (
                    <div
                      key={donation.id}
              ...
              <div className="grid grid-cols-3 gap-4">
                {catalog.map((nft) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`${nftColor} w-full h-full rounded-lg flex items-center justify-center text-white`}>
            ...
            <div className="w-full rounded-[2.15rem] bg-slate-900/65 backdrop-blur-xl text-white p-8 lg:p-10 relative overflow-hidden">
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                <div className="space-y-5">
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-300">Impact NFT</p>
                  <h3 className="text-4xl font-black uppercase tracking-tight">{selectedNft.name}</h3>
                  <p className="text-sm font-semibold text-slate-200">{selectedNft.description}</p>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-300">Projeto</p>
                    <p className="text-xl font-black uppercase">{selectedNft.projectTitle}</p>
                  </div>
                </div>
                ...
                <div className="rounded-3xl border border-white/15 bg-white/5 p-6 text-center space-y-4">
                  <div className="w-24 h-24 mx-auto rounded-2xl border border-white/20 bg-white/10 flex items-center justify-center">
                    <span className="material-icons text-5xl">{selectedNft.icon}</span>
                  </div>
                  <p className="text-xs font-black uppercase tracking-[0.2em]">Raridade: {selectedNft.rarity}</p>
                  <p className="text-xs font-black uppercase tracking-[0.2em]">Badge: {selectedNft.thanks}</p>
