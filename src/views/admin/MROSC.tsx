    ...
    <div className="animate-fade-in">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1 text-sm">
          Analise e valide os relatorios de execucao financeira para garantir a
          conformidade.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              {data.reports.map((report) => (
                <tr className="hover:bg-slate-50" key={report.id}>
                  ...
                  <td className="px-6 py-4 text-slate-700 font-semibold">
                    {Number(report.financialTotalXlm).toLocaleString("pt-BR")}{" "}
                    XLM
                  </td>
                  <td className="px-6 py-4 text-slate-600 font-medium">
        ...
        <div className="p-4 border-t border-slate-100 text-xs text-slate-500">
          Mostrando {data.reports.length} de {data.summary.totalResults}{" "}
          resultados
        </div>
      </div>
    </div>
