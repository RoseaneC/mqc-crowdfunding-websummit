...
This project now includes a backend API workspace at `packages/api`.
...
`dev:full` now runs `npm run init:local` first, which:
- starts `stellar-local` (idempotent)
- starts a local Postgres Docker container (`mqc-postgres-local`)
- applies `packages/api/sql/schema.sql` (schema + seed data)
